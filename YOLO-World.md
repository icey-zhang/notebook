...
为了预训练 YOLO-World，我们主要采用检测或定位数据集，包括 Objects365 (V1)、GQA 和 Flickr30k。根据 [24]，我们排除了 GoldG 中 COCO 数据集的图像（GQA 和 Flickr30k）。检测数据集的注释包含边界框和类别或名词短语。此外，我们还使用图像-文本对，即通过伪标签方法标注的 CC3M 数据集。
